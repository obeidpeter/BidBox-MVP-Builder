...
/**
 * Stores one bounded first-contact request in BidBox's authorised intake
...
/**
 * Requires an active `valo_operations_administrator` grant in a BidBox
...
/**
 * Returns sanitised AI kill-switch, release-gate, capability, recent-run and evaluation state only for a direct BidBox internal operations membership. Client, partner, auditor, restricted-platform-admin and break-glass contexts are denied. Prompt content, document content, provider request identifiers, raw errors and secrets are never returned.
...
/**
 * Copies only the named operator's explicitly selected fields into one existing governed work item under target-version compare-and-swap. Each tenant, project, draft ID and draft-version tuple may be promoted to the target only once, even with a fresh Idempotency-Key. The response is a content-free receipt: no authoritative evidence is created and BidBox cannot delete the browser-local encrypted draft.
...
/**
 * Records what a named person observed and a receipt hash. BidBox does not contact the authority or represent independent issuer verification.
...
export const CreateGrowthQuoteDraftBody = zod.object({
  ...
  "amountMinor": zod.number().min(1).max(createGrowthQuoteDraftBodyAmountMinorMax).describe('Entered by a named operator; never calculated by BidBox.'),
  "validUntil": zod.coerce.date()
})
...
/**
 * Returns a bounded receipt-backed internal renewal register. Internal reminders are register records only; BidBox sends no external message and records no external delivery receipt.
...
export const GetEvidenceRenewalSnapshotResponse = zod.object({
  ...
  "channel": zod.literal("bidbox_evidence_renewal_register"),
  ...
  "authorityNote": zod.literal("This register records a receipt-backed internal due reminder and named-human evidence-renewal workflow. It does not send an external message, contact an issuer or client, approve a pursuit, or claim delivery outside BidBox.")
}).strict()
